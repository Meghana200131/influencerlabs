...
(function($) {
    ...
    var wpwKits = {
        ...
        sugestCat: function() {
            ...
            var data = {
                'action': 'qcld_wb_chatbot_category'
            };
        ...
        scrollTo: function() {
            $(globalwpw.settings.botContainer).animate({
                scrollTop: $(globalwpw.settings.messageWrapper).prop("scrollHeight")
            }, 'slow').parent().find('.slimScrollBar').css({
                'top': $(globalwpw.settings.botContainer).height() + 'px'
            });;
        },
        ...
        shopperMsgDom: function(msg) {
            ...
            var msgContent = '<li class="wp-chat-user-msg">' +
                ...
                '<div class="wp-chatbot-paragraph" style="text-align:center; margin: 0 auto;">' + dateString + '</br></br>' + msg + '</div></li>';
            return msgContent;
        },
        showCart: function() {
            var data = {
                'action': 'qcld_wb_chatbot_show_cart'
            }
        ...
        if (globalwpw.settings.obj.open_product_detail != 1) {
            $(document).on('click', '.wp-chatbot-product a', function(e) {
                ...
                wpwKits.ajax(data).done(function(response) {
                    ...
                    if ($('#wp-chatbot-shortcode-template-container').length > 0) {
                        var data = {
                            'action': 'qcld_wb_chatbot_recently_viewed_products'
                        };
                        wpwKits.ajax(data).done(function(response) {
        ...
        $(document).on('click', '.qcld-chatbot-reset-btn', function(e) {
            ...
            if (actionType == 'yes') {
                ...
                $.wpwbot({
                    obj: wp_chatbot_obj,
                    editor_handler: 0,
                    preLoadingTime: wp_chatbot_obj.botpreloadingtime
                });
